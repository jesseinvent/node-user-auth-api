...
import { sendInvalidOtpError, sendRequestCouldNotBeCompletedError, sendUserAccountNotAvailableError } from "../../helpers/commonAppErrors.js"
...
export default asyncHandler( async (req, res, next) => {
    ...
    if(!errors.isEmpty()) {
        ...
        next(
           new AppError (res, 400, errorMessages)
        )
    }
    ...
    if (user.otp === undefined || user.otp_time_expiry === undefined) {
        next(
            sendInvalidOtpError(res)
        )
    }

    const otpIsValid = await checkIfOtpIsValidAndHasNotExpired(otp, user.otp, user.otp_time_expiry)
    ...
    return sendSuccessApiResponse(res, {
        statusCode: 200,
        message: ["Password reset successful! 😀 Proceed to login with your new password"],
        data: {}
    })
